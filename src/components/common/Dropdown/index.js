...
export default function Dropdown(props) {
    ...
    return (<>
      <div className={`relative crm-dropdown ${!isNoMaxWidth ? "max-w-[500px]" : ""}  ${className ?? ""}`}>
        <select placeholder={placeholder} id={id ?? ""} name={name ?? ""} onClick={() => handleClickDropdown()} ref={selectRef} className={`${dropdownVariantClass} ${borderVariantClass ?? "base-border"}  py-2 pl-3 pr-[30px]`}>
      ...
      <div className={`${!showOptLabel && "hidden"} fixed top-0 left-0 w-[100vw] h-[100vh] z-10" `} onClick={() => {
